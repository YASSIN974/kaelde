...
public class LewdNekoCommand extends NsfwCommand {
    ...
    public LewdNekoCommand(Gson gson) {
        this.gson = gson;
        name = "lewdneko";
    ...
    public void execute(CommandContext context) {
        context.send(context.working() + context.getTranslated("generic.loading"), message -> {
            try {
                ...
                NekosResponse response = gson.fromJson(data, NekosResponse.class);
                if (response.url == null || response.url.isEmpty()) {
                    message.editMessage(context.error() + context.getTranslated("nekos.apierror")).queue();
                } else {
