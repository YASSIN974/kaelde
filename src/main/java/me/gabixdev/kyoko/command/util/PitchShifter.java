...
/****************************************************************************
 *
 * This code was converted to Java from C# by gabixdev
 * https://github.com/gabixdev
 * Source: http://sites.google.com/site/mikescoderama/
 *
 *****************************************************************************/
...
public class PitchShifter {
    ...
    public static float[] PitchShift(float pitchShift, int numSampsToProcess, int fftFrameSize,
        ...
        for (i = 0; i < numSampsToProcess; i++) {
            ...
            if (gRover >= fftFrameSize) {
                ...
                for (k = 0; k < fftFrameSize; k++) {
                    window = -.5 * Math.cos(2.0 * Math.PI * (double) k / (double) fftFrameSize) + .5;
                    gOutputAccum[k] += (float) (2.0 * window * gFFTworksp[(int) (2 * k)] / (fftFrameSize2 * osamp));
                }
                ...
                for (k = 0; k < fftFrameSize; k++) {
                    gOutputAccum[k] = gOutputAccum[(int) (k + stepSize)];
                }

                /* move input FIFO */
                for (k = 0; k < inFifoLatency; k++) gInFIFO[k] = gInFIFO[(int) (k + stepSize)];
            }
        }
        return outdata;
    }
