...
public class PunchCommand extends Command {
    private static final String[] aliases = new String[]{"punch"};
    private Kyoko kyoko;

    public PunchCommand(Kyoko kyoko) {
        this.kyoko = kyoko;
    }
    ...
    public void handle(Message message, Event event, String[] args) throws Throwable {
        ...
        if (args.length == 1) {
            normal.addField(kyoko.getI18n().get(l, "punch.description"), Constants.POWERED_BY_WEEB, false);
        } else {
            ...
            if (message.getMentionedUsers().isEmpty()) {
                normal.addField(String.format(kyoko.getI18n().get(l, "punch.someone"), message.getAuthor().getName(), StringUtil.stripPrefix(kyoko, args[0], message.getContentRaw())), Constants.POWERED_BY_WEEB, false);
            } else {
                ...
                normal.addField(String.format(kyoko.getI18n().get(l, "punch.someone"), message.getAuthor().getName(), String.join(", ", userlist)), Constants.POWERED_BY_WEEB, false);
            }
        }

        Image image = kyoko.getWeeb4j().getRandomImage("punch", HiddenMode.DEFAULT, message.getTextChannel().isNSFW() ? NsfwFilter.ALLOW_NSFW : NsfwFilter.NO_NSFW).execute();
