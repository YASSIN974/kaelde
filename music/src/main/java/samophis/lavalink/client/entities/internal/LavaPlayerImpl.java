...
public class LavaPlayerImpl implements LavaPlayer {
    ...
    public void setPaused(boolean paused) {
        if (this.paused == paused)
            return;
        this.paused = paused;
        node.getSocket().sendText(GsonUtil.toJSON(new SetPausePlayback(guild_id, paused)));
    ...
    public void setVolume(int volume) {
        if (volume < 0 || volume > 150)
            return;
        node.getSocket().sendText(GsonUtil.toJSON(new SetVolume(guild_id, volume)));
        this.volume = volume;
    }
    @Override
    ...
    public void playTrack(@Nonnull AudioTrack track, long startTime, long endTime) {
        ...
        String text = GsonUtil.toJSON(new PlayTrack(guild_id, startTime, endTime, cached.getTrackData()));
    ...
    public void stopTrack() {
        node.getSocket().sendText(GsonUtil.toJSON(new StopPlayback(guild_id)));
    }
    @Override
    public void destroyPlayer() {
        node.getSocket().sendText(GsonUtil.toJSON(new DestroyPlayer(guild_id)));
    }
    @Override
    public void seek(long position) {
        position = Math.max(track.getDuration(), Math.min(0, position));
        node.getSocket().sendText(GsonUtil.toJSON(new SeekTrack(guild_id, position)));
    }
    @Override
    ...
    private void handleTrackPair(TrackPair pair, long start, long end) {
        String data = GsonUtil.toJSON(new PlayTrack(guild_id, start, end, pair.getTrackData()));
