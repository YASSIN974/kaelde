...
public class AudioNodeImpl extends WebSocketAdapter implements AudioNode {
    ...
    public void onDisconnected(WebSocket websocket, WebSocketFrame serverCloseFrame, WebSocketFrame clientCloseFrame, boolean closedByServer) throws Exception {
        ...
        if (closedByServer) {
            if (code == 1000)
                LOGGER.info("Lavalink-Server ({}:{}) closed the connection gracefully with the reason: {}", entry.getRawAddress(), entry.getWebSocketPort(), reason);
            else {
                LOGGER.warn("Lavalink-Server ({}:{}) closed the connection unexpectedly with the reason: {}", entry.getRawAddress(), entry.getWebSocketPort(), reason);
