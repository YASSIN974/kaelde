...
public class ListCommand extends MusicCommand {
    ...
    public void execute(@NotNull CommandContext context) {
        ...
        List<String> pages = StringUtil.createRawPages(queue.getTracks().stream().map(track -> track.getUser().getName().length() + track.getAudioTrack().getInfo().title.length() > 60 ?
                track.getUser().getName() + " queued: " + track.getAudioTrack().getInfo().title.substring(0, 60 - track.getUser().getName().length() - 9) + "..." : track.getUser().getName() + " queued: " + track.getAudioTrack().getInfo().title).collect(Collectors.toList()));
            ...
            protected Message render(int page) {
                ...
                if (player.getPlayingTrack() != null) {
                    if (prev != player.getPlayingTrack()) {
                        ...
                        pageContents = StringUtil.createRawPages(queue.getTracks().stream().map(track -> track.getUser().getName().length() + track.getAudioTrack().getInfo().title.length() > 60 ?
                                track.getUser().getName() + " queued: " + track.getAudioTrack().getInfo().title.substring(0, 60 - track.getUser().getName().length() - 9) + "..." : track.getUser().getName() + " queued: " + track.getAudioTrack().getInfo().title).collect(Collectors.toList()));
                    }
