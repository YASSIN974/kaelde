...
public class AudioWebSocket extends WebSocketAdapter
{
    ...
    public static final JSONArray CODECS = new JSONArray("[{\"name\":\"opus\",\"type\":\"audio\",\"priority\":1000,\"payload_type\":120},{\"name\":\"VP8\",\"type\":\"video\",\"priority\":1000,\"payload_type\":101,\"rtx_payload_type\":102},{\"name\":\"VP9\",\"type\":\"video\",\"priority\":2000,\"payload_type\":103,\"rtx_payload_type\":104}]");
    ...
    private void handleEvent(JSONObject contentAll)
    {
        ...
        switch(opCode)
        {
            ...
            case VoiceCode.READY:
            {
                ...
                // {"op":12,"d":{"audio_ssrc":1,"video_ssrc":2,"rtx_ssrc":3}}
                final JSONObject ssrcUpdate = new JSONObject()
                        .put("audio_ssrc", 1)
                        .put("video_ssrc", 2)
                        .put("rtx_ssrc", 3);
            ...
            case 12: {
                LOG.trace("-> USER_SSRC_UPDATE {}", contentAll);
                final JSONObject content = contentAll.getJSONObject("d");
                final int video_ssrc = content.getInt("video_ssrc");
                final long userId = content.getLong("user_id");

                final User user = getUser(userId);
                if (user == null)
                {
                    AudioConnection.LOG.trace("Got an USER_SSRC_UPDATE for a non-existent User. JSON: {}", contentAll);
                    break;
                }

                audioConnection.updateUserSSRC(video_ssrc, userId, true);
                break;
            }
            case 14: {
