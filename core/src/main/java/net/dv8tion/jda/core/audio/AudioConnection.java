...
public class AudioConnection
{
    ...
    private final TIntLongMap ssrcVideoMap = new TIntLongHashMap();
    ...
    private volatile VideoSendHandler videoSendHandler = null;
    ...
    public void setVideoSendingHandler(VideoSendHandler handler)
    {
        this.videoSendHandler = handler;
        setupSendSystem();
    }
    ...
    public void removeVideoUserSSRC(long userId)
    {
        final AtomicInteger ssrcRef = new AtomicInteger(0);
        final boolean modified = ssrcVideoMap.retainEntries((ssrc, id) ->
        {
            final boolean isEntry = id == userId;
            if (isEntry)
                ssrcRef.set(ssrc);
    ...
    protected void updateUserSSRC(int ssrc, long userId, boolean video) {
        if (video) {
            if (ssrc == 0) {
                LOG.debug("user {} disabled video.", userId);
                removeVideoUserSSRC(userId);
            } else {
                if (ssrcVideoMap.containsKey(ssrc)) {
                    long previousId = ssrcVideoMap.get(ssrc);
                    if (previousId != userId) {
                        LOG.error("Yeah.. So.. JDA received a Video UserSSRC update for an ssrc that already had a User set. Inform DV8FromTheWorld.\nChannelId: {} SSRC: {} oldId: {} newId: {}",
                                channel.getId(), ssrc, previousId, userId);
                    }
                } else {
                    LOG.debug("user {} enabled video.", userId);
                    ssrcVideoMap.put(ssrc, userId);
...
//        setSpeaking(false);
    ...
    {
        if (udpSocket != null && !udpSocket.isClosed() && (sendHandler != null || videoSendHandler != null) && sendSystem == null)
        {
        ...
        else if (sendHandler == null && videoSendHandler == null && sendSystem != null)
        {
            sendSystem.shutdown();
            sendSystem = null;
    ...
    {
        LOG.debug("recv handler: {} recv thread: {}", receiveHandler, receiveThread);

        if (udpSocket != null && !udpSocket.isClosed() && receiveHandler != null && receiveThread == null)
        {
            LOG.debug("setup receive thread");
            setupReceiveThread();
        }
        else if (receiveHandler == null && receiveThread != null)
        {
            LOG.debug("recv handler not null");
            receiveThread.interrupt();
            receiveThread = null;
            ...
            receiveThread = new Thread(AudioManagerImpl.AUDIO_THREADS, () ->
            {
                ...
                while (!udpSocket.isClosed() && !Thread.currentThread().isInterrupted())
                {
                    ...
                    try
                    {
                        ...
                        LOG.debug("recv, len: " + receivedPacket.getLength());
                        ...
                        if (receiveHandler != null && (receiveHandler.canReceiveUser() || receiveHandler.canReceiveCombined()) && webSocket.getSecretKey() != null)
                        {
                            ...
                            LOG.debug("ssrc: {}", ssrc);
                            if (ssrcVideoMap.containsKey(ssrc)) { // video packet
                                LOG.debug("Got video packet! ssrc: {}", ssrc);
                            } else {
                                ...
                                if (userId == ssrcMap.getNoEntryValue()) {
                                    ...
                                    if (!Arrays.equals(audio, silenceBytes))
                                        LOG.debug("Received audio data with an unknown SSRC id ({}). Ignoring", ssrc);
    ...
    private class PacketProvider implements IPacketProvider
    {
        ...
        private ByteBuffer vbuffer = ByteBuffer.allocate(512);
        ...
        @Override
        public DatagramPacket getNextVideoPacket() {
            ByteBuffer buf = getNextVideoPacketRaw();
            return buf == null ? null : getDatagramPacket(buf);
        }

        private ByteBuffer getNextVideoPacketRaw() {
            ByteBuffer nextPacket = null;
            //LOG.debug("videosendhandler = {}", videoSendHandler);
            if (videoSendHandler != null && videoSendHandler.canProvide()) {
                AudioPacket packet = new AudioPacket(seq, (byte) 0x65, timestamp, 2, videoSendHandler.provideVP8Frame());
                int nlen;
                ...
                vbuffer = packet.asEncryptedPacket(vbuffer, webSocket.getSecretKey(), nonceBuffer, nlen);
                return vbuffer;
            }
            return nextPacket;
        }
