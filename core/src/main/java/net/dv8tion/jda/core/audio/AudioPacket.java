...
public class AudioPacket
{
    ...
    private byte payloadType = RTP_PAYLOAD_TYPE;
    ...
    public AudioPacket(byte[] rawPacket) {
        this(rawPacket, (byte) 0x78);
    }

    public AudioPacket(byte[] rawPacket, byte payloadType)
    {
    ...
    public AudioPacket(char seq, int timestamp, int ssrc, byte[] encodedAudio)
    {
        ...
        this.rawPacket = generateRawPacket(seq, payloadType, timestamp, ssrc, encodedAudio);
    }

    public AudioPacket(char seq, byte payloadType, int timestamp, int ssrc, byte[] encodedAudio)
    {
        this.seq = seq;
        this.ssrc = ssrc;
        this.timestamp = timestamp;
    ...
    public ByteBuffer asEncryptedPacket(ByteBuffer buffer, byte[] secretKey, byte[] nonce, int nlen)
    {
        ...
        populateBuffer(seq, payloadType, timestamp, ssrc, encryptedAudio, buffer);
        if (nonce != null)
    ...
    private static byte[] generateRawPacket(char seq, byte payloadType, int timestamp, int ssrc, byte[] data)
    {
        ByteBuffer buffer = ByteBuffer.allocate(RTP_HEADER_BYTE_LENGTH + data.length);
        populateBuffer(seq, payloadType, timestamp, ssrc, data, buffer);
        return buffer.array();
    }

    private static void populateBuffer(char seq, byte payloadType, int timestamp, int ssrc, byte[] data, ByteBuffer buffer)
    {
